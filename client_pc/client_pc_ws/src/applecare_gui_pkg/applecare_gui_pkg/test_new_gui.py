...
class OrchardGUI(QMainWindow):
    def __init__(self,video_node=None,amcl_node=None, gui_manager_node=None):
        ...
        uic.loadUi(os.path.join(relative_path,"applecare_gui_pkg/test_gui.ui"),self)
        ...
        self.task_upload_btn_2.clicked.connect(self.upload_task)
    ...
    def add_pie_chart(self):
        #### send db requeset
        result_list = self.gui_manager_node.request_DB_data(cc=0,table="TaskEvent",
        column="""SUM(CASE
                ...
                SUM(CASE 
                WHEN robot_x > 10 AND robot_x <= 15 
                AND robot_y > 10 AND robot_y <= 15 
                AND target_type_id = 1 
                THEN 1 
                ELSE 0 
                END) AS condition2_count""", where="where event_datetime >= DATE_ADD(NOW(), INTERVAL -1 MONTH)")
        total = 8
        disease1 = result_list[0][0]
        disease2 = result_list[0][1]
