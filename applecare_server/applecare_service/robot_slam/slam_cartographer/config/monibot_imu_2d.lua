...
options = {
  ...
  map_frame = "map",
  tracking_frame = "imu_link", --"base_link", "imu_link"
  published_frame = "odom", -- "map", "base_link", "odom"
  odom_frame = "odom", -- "odom", "odom_filtered"
...
-- TRAJECTORY_BUILDER_2D.imu_gravity_time_constant =  9.8 -- 9.8 
-- TRAJECTORY_BUILDER_2D.imu_sampling_ratio = 1.0
...
TRAJECTORY_BUILDER_2D.motion_filter.max_angle_radians = math.rad(0.1)  -- Lower to improve rotational accuracy

TRAJECTORY_BUILDER_2D.submaps.num_range_data = 80  -- Increase this to make submaps accumulate more data (originally set to about 50)
...
TRAJECTORY_BUILDER_2D.ceres_scan_matcher.translation_weight = 10  -- Increase if map drifts 1e3, 5e2
TRAJECTORY_BUILDER_2D.ceres_scan_matcher.rotation_weight = 0.01  -- Increase for more stable rotation correction  4e2
...
POSE_GRAPH.constraint_builder.min_score = 0.75  -- Reduce slightly if features are being forgotten
POSE_GRAPH.constraint_builder.global_localization_min_score = 0.7  -- Make global localization more permissive if about 0.5
POSE_GRAPH.optimize_every_n_nodes = 5  -- Reduce to run optimizations more frequently for dynamic environments
